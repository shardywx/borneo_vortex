...
def diagnostic_core(exner_cube, theta_cube, thetavd_cube, rhod_cube,
    ...
    lcl = lcl_cube.data
    '''
    # Start July 2020 edit (regularise km. km[0] is the surface exchange coefficient)
    # km[0] is a surface exchange coefficient and did not have to be divided by a layer thickness
    '''
    nbl = km.shape[0]
    ...
    lat0 = 6 # outer latitude of tropical belt  
    lat1 = 3 # inner latitude of tropical belt  
    ...
    # calculate flux coefficients km/dr (July 2020 edit)
    ...
    '''
    vg_tmp = SG.trisolve_blh(km,thick,rhs3v,lat_tropics,vg,BLH)
    ...
    Qsm = Q12.copy()
    Q12 = tropicalfilter(Qsm, latitude)
    Qsm = Q13.copy()
    Q13 = tropicalfilter(Qsm, latitude)
    ...
    # filter tropical information
    Qsm = Q21.copy()
    Q21 = tropicalfilter(Qsm, latitude)
    Qsm = Q23.copy()
    Q23 = tropicalfilter(Qsm, latitude)
    ...
    # filter tropical information
    Qsm = Q31.copy()
    Q31 = tropicalfilter(Qsm, latitude)
    Qsm = Q32.copy()
    Q32 = tropicalfilter(Qsm, latitude)
    ...
    # Calculate G, eq. (62)
    (G1, G2, G3) = SG.CalcG(Q11R, Q12, Q21, Q22R, Q31, Q32,
...
# calculate P^{-1}G
    ...
    for outeriter in range(0, outer):
        ...
        (G1, G2, G3) = SG.CalcG(Q11R, Q12, Q21, Q22R, Q31, Q32,
                                ug, vg, g, thetav, uinc, vinc, heat, nbl, ny, coriolis,
                                updiffcoeff, diagdiffcoeff, downdiffcoeff)
        (G1_, G2_, G3_) = SG.CalcG2(Q11R, Q31, Q32,
                                ug, vg, g, thetav, uinc, vinc, heat, nbl, ny, coriolis,
                                updiffcoeff, diagdiffcoeff, downdiffcoeff)
        # Spread G3 in vertical
        if cloud > 0:
        ...
        # Use if only the response to forcingl increments required
        if heating > 0:
            (G1, G2, G3) = SG.CalcGinc(ug, vg, g, thetav, uinc, vinc, heat, nbl, ny, coriolis,
                            updiffcoeff, diagdiffcoeff, downdiffcoeff)
        ...
        # Merge old and new u,v,w in some latitudinal range (edit to link with 'outer' and 'inner'?)
    ...
    # call tropicalfilter again, before writing values to Iris cubes
    Qsm = tropicalfilter(dpidt, latitude)
    dpidt = Qsm
    Qsm = tropicalfilter(dugdt, latitude)
    dugdt = Qsm
    Qsm = tropicalfilter(dvgdt, latitude)
    dvgdt = Qsm
    ...
    return ug_cube, vg_cube, dpidt_cube, dugdt_cube, dvgdt_cube, dthetavdt_cube,\
           u_cube, v_cube, w_cube, balheat1_cube, balheat2_cube, ug_um_cube, vg_um_cube
